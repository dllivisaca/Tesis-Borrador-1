...
$query = "
    ...
    WHERE 
    (
        (TIMESTAMP(CONCAT(fecha, ' ', hora_inicio)) = DATE_ADD('$fecha_hora_actual', INTERVAL 24 HOUR) AND recordatorio_24hrs_enviado = 0)
        OR
        (TIMESTAMP(CONCAT(fecha, ' ', hora_inicio)) = DATE_ADD('$fecha_hora_actual', INTERVAL 3 HOUR) AND recordatorio_3hrs_enviado = 0)
    )
    AND estado = 'pendiente'
"; */
